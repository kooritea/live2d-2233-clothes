    ...
    let req = https.get({
      ...
      response.on('end',() => {
        if(response.statusCode === 200){
          fs.existsSync(`${__dirname}/../dist/${name}/${ClothesName}`,(exists)=>{
            if(exists){
              fs.writeFile(`${__dirname}/../dist/${name}/${ClothesName}/texture_0${i}.png`,body,'binary',(err)=>{});
            }
            else{
              fs.mkdirSync(`${__dirname}/../dist/${name}/${ClothesName}`)
              fs.writeFile(`${__dirname}/../dist/${name}/${ClothesName}/texture_0${i}.png`,body,'binary',(err)=>{});
            }
          })
        }
